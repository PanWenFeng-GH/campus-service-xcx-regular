...
zplayer.prototype.init = function() {
    ...
    ht +='<i title="列表循环" class="player-type-icon z-icon z-icon-retweet"></i>'
        +'					</div>'
        +'				</div>'
        +'			</div>'
        +'		</div>'
    ...
    this.playTypeIcon.addEventListener("click", function() {
		...
		if(playTypes[playType]=="z-icon-retweet")ShowTip("列表循环", 'info');
		if(playTypes[playType]=="z-icon-retweet-one")ShowTip("单曲循环", 'info');
		if(playTypes[playType]=="z-icon-random")ShowTip("随机播放", 'info');
		if(playTypes[playType]=="z-icon-reorder-list")ShowTip("顺序播放", 'info');	
		
		if(playTypes[playType]=="z-icon-retweet") p.playTypeIcon.title="列表循环";
		if(playTypes[playType]=="z-icon-retweet-one")p.playTypeIcon.title="单曲循环";
		if(playTypes[playType]=="z-icon-random")p.playTypeIcon.title="随机播放";
		if(playTypes[playType]=="z-icon-reorder-list")p.playTypeIcon.title="顺序播放";			
    })
...
function playSwitch4s(){
	curPlayType=playType;
	var audi4s = document.createElement("audio");
	audi4s.src = "/video/4s.mp3";
	audi4s.loop = 0;
	audi4s.preload = "metadata";
	audi4s.play();
}
}
